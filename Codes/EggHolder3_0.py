...
def fn_my_controller():
    ...
    while (True):
        ...
        left_zero_x2_point = fn_max((x2_minimum_point - _epsilon), x2_lower_bound)
        left_zero_x2_point_prev = left_zero_x2_point
        count_left_x2 = 1
        # Left search for x2
        while (True):
            left_zero_point_2 = fn_find_cutoffs(x1_minimum_point, x1_minimum_point, left_zero_x2_point,
                                                x2_minimum_point, str('left x2 (' + str(count_left_x2)))
            left_zero_x2_point = left_zero_point_2[1]
            if (round(left_zero_x2_point_prev, _rounding_constant) == round(left_zero_x2_point, _rounding_constant)):
                break
            else:
                left_zero_x2_point_prev = left_zero_x2_point
        ...
        new_x2_bounds_2 = (x2_lower_bound, left_zero_x2_point)
        ...
        new_x2_bounds_3 = (left_zero_x2_point, x2_upper_bound)

        # right zero point keeping x2 constant
        right_zero_x2_point = fn_min((x2_minimum_point + _epsilon), x2_upper_bound)
        right_zero_x2_point_prev = right_zero_x2_point
        count_right_x2 = 1
        while (True):
            right_zero_point_2 = fn_find_cutoffs(x1_minimum_point, x1_minimum_point, x2_minimum_point,
                                                 right_zero_x2_point, str('right x2 (' + str(count_right_x2)))
            right_zero_x2_point = right_zero_point_2[1]
            if (round(right_zero_x2_point_prev, _rounding_constant) == round(right_zero_x2_point, _rounding_constant)):
                break
            else:
                right_zero_x2_point_prev = right_zero_x2_point
        ...
        new_x2_bounds_4 = (right_zero_x2_point, x2_upper_bound)

        #print([new_x1_bounds_1, new_x2_bounds_1])
        if not (fn_check_ommited_regions([new_x1_bounds_1[0], new_x2_bounds_1[0]]) and fn_check_ommited_regions(
                [new_x1_bounds_1[1], new_x2_bounds_1[1]])) and not (
                round(new_x1_bounds_1[0]) == round(new_x1_bounds_1[1]) and round(new_x2_bounds_1[0]) == round(
                new_x2_bounds_1[1])) and (round(new_x1_bounds_1[0]+0.5, 0) != round(new_x1_bounds_1[1]+0.5, 0)) and (round(new_x2_bounds_1[0]+0.5, 0) != round(new_x2_bounds_1[1]+0.5, 0)):
            _bounds_list.append([new_x1_bounds_1, new_x2_bounds_1])
            print("1 : " + str([new_x1_bounds_1, new_x2_bounds_1]))

        if not (fn_check_ommited_regions([new_x1_bounds_2[0], new_x2_bounds_2[0]]) and fn_check_ommited_regions(
                [new_x1_bounds_2[1], new_x2_bounds_2[1]])) and not (
                round(new_x1_bounds_2[0]) == round(new_x1_bounds_2[1]) and round(new_x2_bounds_2[0]) == round(
                new_x2_bounds_2[1])) and (round(new_x1_bounds_2[0]+0.5,0) != round(new_x1_bounds_2[1]+0.5,0)) and (round(new_x2_bounds_2[0]+0.5,0) != round(new_x2_bounds_2[1]+0.5,0)):
            _bounds_list.append([new_x1_bounds_2, new_x2_bounds_2])
            print("2 : " + str([new_x1_bounds_2, new_x2_bounds_2]))

        if not (fn_check_ommited_regions([new_x1_bounds_3[0], new_x2_bounds_3[0]]) and fn_check_ommited_regions(
                [new_x1_bounds_3[1], new_x2_bounds_3[1]])) and not (
                round(new_x1_bounds_3[0]) == round(new_x1_bounds_3[1]) and round(new_x2_bounds_3[0]) == round(
                new_x2_bounds_3[1])) and (round(new_x1_bounds_3[0]+0.5,0) != round(new_x1_bounds_3[1]+0.5,0)) and (round(new_x2_bounds_3[0]+0.5,0) != round(new_x2_bounds_3[1]+0.5,0)):
            _bounds_list.append([new_x1_bounds_3, new_x2_bounds_3])
            print("3 : " + str([new_x1_bounds_3, new_x2_bounds_3]))

        if not (fn_check_ommited_regions([new_x1_bounds_4[0], new_x2_bounds_4[0]]) and fn_check_ommited_regions(
                [new_x1_bounds_4[1], new_x2_bounds_4[1]])) and not (
                round(new_x1_bounds_4[0]) == round(new_x1_bounds_4[1]) and round(new_x2_bounds_4[0]) == round(
                new_x2_bounds_4[1])) and (round(new_x1_bounds_4[0]+0.5,0) != round(new_x1_bounds_4[1]+0.5,0)) and (round(new_x2_bounds_4[0]+0.5,0) != round(new_x2_bounds_4[1]+0.5,0)):
            _bounds_list.append([new_x1_bounds_4, new_x2_bounds_4])
            print("4 : " + str([new_x1_bounds_4, new_x2_bounds_4]))

        #print("Stack 2 : (" + str(len(_bounds_list)) + ") >> " + str(_bounds_list))
        _omitted_regions.append([left_zero_x1_point, right_zero_x1_point, left_zero_x2_point, right_zero_x2_point])
...
def fn_check_ommited_regions(check_point):
    global _omitted_regions
    #print("fn_check_ommited_regions")
    for region in _omitted_regions:
        ...
        if round(region_point_1+0.5, 0) <= round(check_point[0]+0.5, 0) and round(
                region_point_2+0.5, 0) >= round(check_point[0]+0.5, 0) and round(
                region_point_3+0.5, 0) <= round(check_point[1]+0.5, 0) and round(
                region_point_4+0.5, 0) >= round(check_point[1]+0.5, 0):
            #print("Yes")
            return True
